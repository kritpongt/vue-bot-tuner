<script lang="ts"></script>

<template>
	<div class="bg-cover bg-fixed bg-[url(/bg/resort_festival.jpg)]">
		<div class="navbar bg-base-100 shadow-sm">
			<div class="flex-none">
				<!-- <button class="btn btn-square btn-ghost">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="inline-block h-5 w-5 stroke-current"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path> </svg>
				</button> -->
			</div>
			<div class="flex-1">
				<RouterLink class="btn btn-ghost text-xl" to="/">Bot Tuner</RouterLink>
			</div>
			<div class="flex-none">
				<ul class="menu menu-horizontal px-2">
					<!-- <li><a>CoffeeMe</a></li> -->
					<!-- <li><RouterLink to="/backoffice">Backoffice</RouterLink></li> -->
				</ul>
				<!-- <button class="btn btn-square btn-ghost">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="inline-block h-5 w-5 stroke-current"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h.01M12 12h.01M19 12h.01M6 12a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0z"></path> </svg>
				</button> -->
			</div>
		</div>

		<div class="w-full px-4 pt-12 xl:px-20 2xl:px-60">
			<slot></slot>
		</div>

		<div class="h-20"></div>
		<footer class="footer sm:footer-horizontal footer-center bg-base-300 text-base-content p-4">
			<aside>
				<p>© {{ new Date().getFullYear() }} Overtuning - All right reserved.</p>
			</aside>
		</footer>
	</div>
</template>
